...
from BackwardGAN import BackwardGAN
...
flags.DEFINE_string("dataset", "noise images", "The name of dataset [celebA, mnist, lsun]")
flags.DEFINE_string("checkpoint_dir", "checkpoint/", "Directory name to save the checkpoints [checkpoint]")
flags.DEFINE_string("sample_dir", "samples/lung", "Directory name to save the image samples [samples]")
flags.DEFINE_string("dataset_test", "test", "Directory name of testing samples")
flags.DEFINE_string("dataset_target", "clear images", "Directory name of target(generated) images")
...
def main(_):
    ...
    with tf.Session(config=run_config) as sess:
        ...
        if FLAGS.train:
            backwardGAN.train(FLAGS)
        else:
            if not backwardGAN.load(FLAGS.checkpoint_dir)[0]:
                raise Exception("[!] Train a model first, then run test mode")
            backwardGAN.test(FLAGS)
