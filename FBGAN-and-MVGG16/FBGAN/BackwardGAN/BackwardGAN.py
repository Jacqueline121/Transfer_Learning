...
class BackwardGAN(object):
    ...
    def test(self, config):
        ...
        batch_idxs = min(len(self.data_test), config.train_size) // config.batch_size
        for idx in xrange(0, batch_idxs):
            batch_files = self.data_test[idx * config.batch_size:(idx + 1) * config.batch_size]
            batch = [
                get_image(batch_file,
                          input_height=self.input_height,
                          input_width=self.input_width,
            ...
            save_path ='output/' + str(idx)+'.png'
            imsave(samples, [samples.shape[0], samples.shape[1]],save_path)
